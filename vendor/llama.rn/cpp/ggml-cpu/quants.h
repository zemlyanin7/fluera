...
void quantize_row_stq1_0(const float * LM_GGML_RESTRICT x, void * LM_GGML_RESTRICT y, int64_t k);
...
void lm_ggml_vec_dot_stq1_0_q8_K(int n, float * LM_GGML_RESTRICT s, size_t bs, const void * LM_GGML_RESTRICT vx, size_t bx, const void * LM_GGML_RESTRICT vy, size_t by, int nrc);
#if defined(__aarch64__) || defined(__arm__) || defined(_M_ARM) || defined(_M_ARM64)
// STQ1_0 ARM NEON path uses planar Q8_K activation; the in-place repack
// is only declared on ARM and called from lm_ggml_compute_forward_mul_mat.
