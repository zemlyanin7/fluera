...
void quantize_row_stq1_0_ref(const float * LM_GGML_RESTRICT x, block_stq1_0 * LM_GGML_RESTRICT y, int64_t k) {
    assert(k % QK_K == 0);
    const int64_t nb = k / QK_K;
    ...
    for (int64_t i = 0; i < nb; ++i) {
        ...
        y[i].d = LM_GGML_FP32_TO_FP16(amax);
...
size_t quantize_stq1_0(const float * LM_GGML_RESTRICT src, void * LM_GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    (void)quant_weights; // not used
    const size_t row_size = lm_ggml_row_size(LM_GGML_TYPE_STQ1_0, n_per_row);
...
void dequantize_row_stq1_0(const block_stq1_0 * LM_GGML_RESTRICT x, float * LM_GGML_RESTRICT y, int64_t k) {
    assert(k % QK_K == 0);
    const int64_t nb = k / QK_K;
