// FB2 файлы часто в windows-1251 (русская локализация). EPUB всегда UTF-8.
// Node/Hermes TextDecoder windows-1251 НЕ всегда доступен (зависит от ICU build),
// поэтому ИСПОЛЬЗУЕМ inline lookup table — 256 mappings, ~2KB кода, deterministic.
export type SupportedEncoding = 'utf-8' | 'windows-1251';

const ENCODING_RX = /<\?xml\s+[^?]*encoding=["']([^"']+)["']/i;

// windows-1251 → unicode table. Записи 0x80..0xFF (lower range = ASCII).
// Source: https://encoding.spec.whatwg.org/index-windows-1251.txt
const CP1251_TABLE: number[] = [
  0x0402, 0x0403, 0x201a, 0x0453, 0x201e, 0x2026, 0x2020, 0x2021,
  0x20ac, 0x2030, 0x0409, 0x2039, 0x040a, 0x040c, 0x040b, 0x040f,
  0x0452, 0x2018, 0x2019, 0x201c, 0x201d, 0x2022, 0x2013, 0x2014,
  0xfffd, 0x2122, 0x0459, 0x203a, 0x045a, 0x045c, 0x045b, 0x045f,
  0x00a0, 0x040e, 0x045e, 0x0408, 0x00a4, 0x0490, 0x00a6, 0x00a7,
  0x0401, 0x00a9, 0x0404, 0x00ab, 0x00ac, 0x00ad, 0x00ae, 0x0407,
  0x00b0, 0x00b1, 0x0406, 0x0456, 0x0491, 0x00b5, 0x00b6, 0x00b7,
  0x0451, 0x2116, 0x0454, 0x00bb, 0x0458, 0x0405, 0x0455, 0x0457,
  0x0410, 0x0411, 0x0412, 0x0413, 0x0414, 0x0415, 0x0416, 0x0417,
  0x0418, 0x0419, 0x041a, 0x041b, 0x041c, 0x041d, 0x041e, 0x041f,
  0x0420, 0x0421, 0x0422, 0x0423, 0x0424, 0x0425, 0x0426, 0x0427,
  0x0428, 0x0429, 0x042a, 0x042b, 0x042c, 0x042d, 0x042e, 0x042f,
  0x0430, 0x0431, 0x0432, 0x0433, 0x0434, 0x0435, 0x0436, 0x0437,
  0x0438, 0x0439, 0x043a, 0x043b, 0x043c, 0x043d, 0x043e, 0x043f,
  0x0440, 0x0441, 0x0442, 0x0443, 0x0444, 0x0445, 0x0446, 0x0447,
  0x0448, 0x0449, 0x044a, 0x044b, 0x044c, 0x044d, 0x044e, 0x044f,
];

function decodeCp1251(bytes: Uint8Array): string {
  let out = '';
  for (let i = 0; i < bytes.length; i++) {
    const b = bytes[i] ?? 0;
    if (b < 0x80) {
      out += String.fromCharCode(b);
    } else {
      const code = CP1251_TABLE[b - 0x80] ?? 0xfffd;
      out += String.fromCharCode(code);
    }
  }
  return out;
}

export function detectXmlEncoding(bytes: Uint8Array): SupportedEncoding {
  const head = String.fromCharCode(...bytes.subarray(0, Math.min(256, bytes.length)));
  const m = ENCODING_RX.exec(head);
  if (!m) return 'utf-8';
  const enc = (m[1] ?? '').toLowerCase();
  if (enc === 'utf-8' || enc === 'utf8') return 'utf-8';
  if (enc === 'windows-1251' || enc === 'cp1251') return 'windows-1251';
  return 'utf-8';
}

export function decodeBytes(bytes: Uint8Array, encoding: SupportedEncoding): string {
  if (encoding === 'utf-8') {
    return new TextDecoder('utf-8').decode(bytes);
  }
  if (encoding === 'windows-1251') {
    return decodeCp1251(bytes);
  }
  throw new Error(`Unsupported encoding: ${encoding}`);
}
